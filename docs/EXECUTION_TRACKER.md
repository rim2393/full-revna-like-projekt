...
| PR-007 | WireGuard/AWG traffic accounting and honest torrent policy | PARTIAL | WG/AWG node-agent reports real transfer deltas in `rx_bytes`/`tx_bytes`; reset clears WG baseline; torrent-blocker on WG/AWG is blocked before queue instead of faked | Local implementation adds `wg/awg show <interface> transfer` collection with hashed peer state and delta accounting, exports the collector, merges deltas into runtime metrics for the existing API traffic summary, and resets the WG baseline during `node.traffic.reset`. Backend apply now rejects blocking `torrent-blocker` policy for WireGuard/AWG with `422 wireguard_torrent_policy_unsupported` before queuing a node command. Local gates passed on 2026-06-04: full node-agent `node --test` (`122 passed`), API `ruff` on changed protocol tests/service, and focused API pytest `tests/test_apply_profile_to_node_routes.py tests/test_node_outbound_config.py -k "wireguard or torrent"` (`6 passed`). Pending: signed `v0.1.131` release, prod panel/node upgrade, and live metric smoke on real `node-01`; keep `PARTIAL` until deployed evidence is recorded. |
