...
| --- | --- |
| Latest production release | `v0.1.131` WireGuard/AWG traffic-accounting and honest torrent-policy hardening deployed through the signed public manifest on the real panel and node after GitHub-hosted Actions remained blocked by account billing/spending |
| Product repo head | latest pushed `main` includes `d5daa33` WireGuard/AWG runtime traffic deltas and torrent-policy guard plus the prior auth-shell localization, stale profile cleanup, PR-006 signed prod client compatibility evidence, IKEv2 PKI SAN hardening, API-key principal email hardening, and API logging module rename |
| Public installer manifest | `rim2393/lumen_vpn@2796b2f` publishes signed `v0.1.131` manifest and current public release verification key |
| Prod health | Panel API container `lumen-api-1` on digest-pinned `v0.1.131@sha256:e5952cd6d5697022e95e457b89293a78f522b3908e6fc530f5d658d1e3950ab4` and Docker health `healthy`; prod `lumen-web` on digest-pinned `v0.1.130@sha256:895904779300c0eefb8fd5f359a7e7c32f6784a4dd57895984a535e686ca2a3f` and Docker health `healthy`; prod subscription page on digest-pinned `v0.1.120@sha256:5309dacd7f48ed587b931004f34ac6c5f13523c2a6554d62cfa191a0006cd601`; node VPS `lumen-node-node-agent-1` on digest-pinned `v0.1.131@sha256:4156355285e2ff303c634cb7c3b4c52c905a69f72f3be167dd4a5f08ba492fb0`; public `https://panel.lumentech.tel/api/v1/health/ready` returns `200`; official `upgrade.sh` applied `LUMEN_VERSION=v0.1.131` with encrypted backup `lumen-backup-20260604T120213Z.tar.gz.enc`; panel `/tmp/lumen-* = 0`, node `/tmp/lumen-* = 0`, node VPS contains only `/opt/lumen-node` runtime/config/state/policies files and no installer/admin checkout |
...
| PR-008 | IKEv2 traffic accounting | PARTIAL | IKEv2 node-agent reports real strongSwan/XFRM transfer deltas into shared `rx_bytes`/`tx_bytes`; reset clears IKEv2 baseline; prod release and live node metric smoke still required | Local implementation added `apps/node-agent/src/ikev2-traffic.js`: preferred collector reads real `swanctl --list-sas --raw` counters, fallback reads `ip -s xfrm state`, first observation establishes a baseline, later heartbeats emit deltas to shared traffic fields plus `ikev2_*` cumulative diagnostics, and `node.traffic.reset` clears the IKEv2 baseline together with telemetry and WireGuard. Local gate passed on 2026-06-04: `npm --prefix apps/node-agent test` (`128 passed`). Not DONE until versioned image promotion, official upgrade, and real prod metric evidence are recorded. |
